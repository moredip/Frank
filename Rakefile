...
task :build_iphone_lib do
  sh "xcodebuild -project Frank.xcodeproj -target Frank -configuration Release -sdk iphoneos#{discover_latest_sdk_version} BUILD_DIR=build clean build"
end

desc "Build the i386 library"
task :build_simulator_lib do
  sh "xcodebuild -project Frank.xcodeproj -target Frank -configuration Release -sdk iphonesimulator#{discover_latest_sdk_version} BUILD_DIR=build clean build"
end
