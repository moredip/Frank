...
#define TAG_START_VALUE_FIELD (2)
#define TAG_DECREMENT_FIELD (3)

#define NUMBERS	@"0123456789"

@implementation CountDownViewController

@synthesize managedObjectContext, countDownObject;
...
- (void)viewDidLoad 
{
	...
	isNewObject = (nil == countDownObject);
	
	if (isNewObject)
	{
		self.navigationItem.title = @"Count Down";
		
		countDownObject = [NSEntityDescription insertNewObjectForEntityForName:@"Counter" inManagedObjectContext:managedObjectContext];
		[countDownObject setValue:[NSNumber numberWithInt:COUNTDOWN] forKey:@"Type"];
		[countDownObject setValue:[NSNumber numberWithInt:0] forKey:@"Value"];
	...
	{
		self.navigationItem.title = (NSString *) [countDownObject valueForKey:@"Name"];
	}
}
...
-(void) save:(id) sender
{
	...
	if (nil != textFieldStartValue)
		[textFieldStartValue resignFirstResponder];
	if (nil != textFieldDecrement)
		[textFieldDecrement resignFirstResponder];
	...
	{		
		...
		[countDownObject setValue:group forKey:@"Group"];
	}
	else 
	{
		...
		case 1:
		{
			switch (indexPath.row)
			{
				case 0:
					...
					break;
				case 1:
					nvCell.value.tag = TAG_START_VALUE_FIELD;
					nvCell.name.text = @"Starting Value:";
					nvCell.value.placeholder = @"Starting Value";
					nvCell.value.keyboardType = UIKeyboardTypeNumbersAndPunctuation;					
					
					textFieldStartValue = nvCell.value;
					break;
				case 2:
					nvCell.value.tag = TAG_DECREMENT_FIELD;
					nvCell.name.text = @"Decrement:";
					nvCell.value.placeholder = @"Decrement";
					nvCell.value.keyboardType = UIKeyboardTypeNumbersAndPunctuation;					
					
					textFieldDecrement = nvCell.value;
					break;
			}
		}
	}
...
{
	if (textField.tag == TAG_START_VALUE_FIELD || textField.tag == TAG_DECREMENT_FIELD)
	{
		NSCharacterSet *cs;
		NSString *filtered;
...
- (void)textFieldDidEndEditing:(UITextField *)textField
{
	...
	{
		case TAG_NAME_FIELD:
			[countDownObject setValue:[NSString stringWithString:textField.text] forKey:@"Name"];
			self.navigationItem.title = (NSString *) [countDownObject valueForKey:@"Name"];
			break;
		case TAG_DECREMENT_FIELD:
			[countDownObject setValue:[NSNumber numberWithInt:textField.text.intValue] forKey:@"Increment"];
			break;
		case TAG_START_VALUE_FIELD:
		{
			int value = 0;
			if (textField.text.length > 0)
				value = textField.text.intValue;

			[countDownObject setValue:[NSNumber numberWithInt:value] forKey:@"MaxValue"];
			[countDownObject setValue:[NSNumber numberWithInt:value] forKey:@"Value"];
			break;
		}
	}
}
